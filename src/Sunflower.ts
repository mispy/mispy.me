...
class Sunflower {
    ...
    changeSpeed(multiplier: number) {
        for (let i = 0; i < this.points.length; i++) {
            ...
            this.points[i].pointAngleMultiplier = pointAngleMultiplier
        }
    }
    ...
    expandRipples(deltaTime?: number) {
...
class Sunflower3DRenderer {
    ...
    render = (_time: number) => {
        ...
        const deltaTime = _time - this.lastTime
        this.lastTime = _time
        const gl = this.gl
        ...
        this.sunflower.nextFrame(deltaTime)
        requestAnimationFrame(this.render)
    }
}
